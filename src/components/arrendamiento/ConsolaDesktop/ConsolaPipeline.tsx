...
export const ConsolaPipeline = () => {
  const { expediente, cambiarFase } = useArrendamiento();

  const fases = fasesOrdenadas.map(id => ({
    id,
    label: faseLabels[id],
  }));
  ...
  return (
    <div className="flex-1 p-6 overflow-y-auto bg-slate-900/50">
      <div className="mb-6">
        <h2 className="text-lg font-semibold text-slate-100 mb-1">Pipeline de Contratación</h2>
        <p className="text-sm text-slate-400">Proceso guiado por Certy: Gestor CLM - Prestador Cualificado Servicios de Confianza</p>
      </div>
      ...
      <div className="flex gap-4 overflow-x-auto pb-4">
        {fases.map((fase, index) => (
          <div key={fase.id} className="flex items-center gap-4 flex-shrink-0">
            <PipelineCard
              fase={fase.id as FaseArrendamiento}
              titulo={fase.label}
              cantidad={1}
              activo={expediente.fase === fase.id}
              completado={fasesOrdenadas.indexOf(expediente.fase) > index}
              dias={calcularDias()}
            />
        ...
        <div className="flex flex-wrap gap-2">
          {fases.map((fase) => (
            <Button
              ...
              size="sm"
              onClick={() => cambiarFase(fase.id as FaseArrendamiento)}
              className={cn(
                "text-xs",
